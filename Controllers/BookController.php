...
class BookController extends Controller
{
    ...
    public function searchHotels(Request $request)
    {
        ...
        {
            $contacts = Book::all(); // Contacts テーブルからデータを取得
            $reservations = Reservation::all(); // Reservations テーブルからデータを取得
            $users = User::all(); // Users テーブルからデータを取得        
            return view('book.data', compact('contacts','reservations','users'));
        }
        ...
        {
            $credentials = $request->only('email', 'password', 'security_question', 'security_answer');
            ...
            if (Auth::attempt($credentials)) {
                ...
                if ($user->role === 'admin') {
                    return response()->json(['success' => true, 'redirect' => route('data')]);
                } else {
                    return response()->json(['success' => true, 'redirect' => route('main-page')]);
                }
            } else {
                // ログイン失敗時の処理
