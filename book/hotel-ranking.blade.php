...
<body>
    ...
    <p class="title">Hotel Ranking</h3>




        <div class="hotel-list">
            @if (count($filteredRankings) > 0)
                @foreach ($filteredRankings as $ranking)
                    @if (isset($ranking['Ranking']['hotels']))
                        @foreach ($ranking['Ranking']['hotels'] as $hotel)
                            <div class="hotel-item">
                                <img src="{{ $hotel['hotel']['hotelImageUrl'] }}" alt="{{ $hotel['hotel']['hotelName'] }}" class="hotel-image">
                                <h2><a href="{{ $hotel['hotel']['hotelInformationUrl'] }}" target="_blank">{{ $hotel['hotel']['hotelName'] }}</a></h2>
                                <div class="ranking-number"><p>ランキング：{{ $hotel['hotel']['rank'] }}</p></div>
                                <p>エリア：{{ $hotel['hotel']['middleClassName'] }}</p>
                                <p>レビュー数：{{ $hotel['hotel']['reviewCount'] }}</p>
                                <p>評価：{{ $hotel['hotel']['reviewAverage'] }}</p>
                                <p>詳細情報：{{ $hotel['hotel']['hotelInformationUrl'] }}</p>
                                <button class="select-button" data-hotel-id="{{ $hotel['hotel']['hotelNo'] }}">選択</button>
                            </div>
                        @endforeach
                    @endif
                @endforeach
